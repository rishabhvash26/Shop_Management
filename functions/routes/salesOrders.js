...
function computeLineItem(product, productId, qty, discountPercent) {
...
router.post('/', async (req, res) => {
  const { customerName, items, taxPercent } = req.body;
  ...
  try {
    const result = await db.runTransaction(async (tx) => {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        lineItems.push(computeLineItem(product, productId, requestedQty, discountPercent));
      }
...
// PUT /api/sales-orders/:id - full edit (customer, items, discount, tax, payment method).
...
// refund, and settles it either as a reduction of what's owed (credit orders) or as cash
// handed back to the customer (already-paid orders).
async function processReturnOrCancel(orderId, requestedItems, mode) {
  return db.runTransaction(async (tx) => {
    ...
    for (const [productId, returnQty] of returnMap.entries()) {
      ...
      const netUnitPrice = Number(line.lineTotal) / Number(line.qty);
      refundAmount += netUnitPrice * returnQty;
